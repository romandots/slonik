...
> **YAML-индентация важна.** Каждый проект под `projects:` начинается с
> `  - slug:` (ровно два пробела слева, не таб). Если перепутать отступ
> (например, написать `- slug:` от начала строки), bootstrap откажется
> стартовать с сообщением вида:
>
> ```
> Invalid YAML in bootstrap manifest (/app/bootstrap/manifest.yaml):
>   line 35, col 1: A block sequence may not be used as an implicit map key
>
> Hint: check indentation around that line — list items under `projects:` must
> start with `  - ` (two spaces). YAML is whitespace-sensitive.
> ```
>
> Указанная строка — место сбоя; почини отступы вокруг неё и повтори
> `make bootstrap`.
...
- **Спецсимволы в `project.name`** — Plane v1.3.0 разрешает в имени
  проекта только `[A-Za-z0-9 _-]+` (буквы, цифры, пробелы, `_`, `-`).
  Точки, em-dash (`—`), двоеточия, slash и прочая пунктуация режутся
  бэкендом 400-ой: `"Project name cannot contain special characters."`
  Bootstrap провалидирует это локально (`src/bootstrap/manifest.ts`,
  regex'ом на `projects[].name`) — так что упасть на zod-валидации с
  понятным сообщением «Plane project name must match …» можно ещё до
  похода в Plane. Если в манифесте несколько проектов, цикл устойчив:
  при сбое одного проекта (валидация / сеть / Plane 4xx) bootstrap не
  падает, помечает проект как `FAILED` в отчёте и идёт дальше; CLI
  вернёт ненулевой exit code и список упавших проектов.
...
- **`429 RATE_LIMIT_EXCEEDED`** — `PLANE_API_KEY_RATE_LIMIT` по умолчанию
  `60/minute`, а bootstrap нескольких проектов спокойно делает 130+
  write-вызовов (5 проектов × 12 states + 14 labels + identity invites).
  MCP-клиент сам ретраит 429 с уважением `Retry-After` (full-jitter
  exp. backoff 1–30s между попытками) до `MCP_RETRY_ATTEMPTS_429`
  раз (дефолт `10`) — для штатного bootstrap'а этого хватает. Если всё
  равно упёрся в потолок ретраев, в логе будет понятное сообщение
  «Plane rate limit exceeded after N attempts over Xs … Raise
  PLANE_API_KEY_RATE_LIMIT in .env»: подними `PLANE_API_KEY_RATE_LIMIT=300/minute`
  в `.env`, пересоздай `plane-api` (`docker compose up -d plane-api`) и
  повтори `make bootstrap` — он идемпотентен. Размер бюджета на 429
  тоже регулируется через `MCP_RETRY_ATTEMPTS_429`.
...
> `docs/` — `make up` выдаст `No rule to make target 'up'`.
...
5. Заведёт 7 agent-identities (`analyst-agent`, `developer-agent`,
   `security-auditor-agent`, `code-review-agent`, `qa-agent`, `doc-agent`,
   `merger-agent`). По умолчанию `MCP_AGENT_IDENTITY_MODE=per_user` — bootstrap
   инвайтит 7 пользователей; при ошибке инвайта автоматически падает на
   `single_bot`, маппинг `role → plane_user_id` пишется в
   `mcp_data/identity.sqlite`. Этот же файл — primary-источник whitelist'а
   для валидации `X-Agent-Identity` в рантайме (fallback — сам манифест);
   список ролей в коде не хардкодится, см.
   [SPEC.md §5.5](./SPEC.md#55-agent-identity).
...
- `X-Agent-Identity: <role>` — одна из ролей bootstrap'а (по умолчанию 7:
  `analyst-agent`, `developer-agent`, `security-auditor-agent`,
  `code-review-agent`, `qa-agent`, `doc-agent`, `merger-agent`; whitelist
  собирается на старте из `identity.sqlite` / `bootstrap/manifest.yaml`,
  см. [SPEC.md §5.5](./SPEC.md#55-agent-identity)). Без него или с неизвестной
  ролью MCP отдаст `400 IDENTITY_REQUIRED`.
