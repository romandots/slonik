...
Состояния и переходы (примерный флоу):
...
только `merge-agent` после `Merging`.
...
7. **Никогда не закрывай чужую работу.** `transition → Done` делает только
   та роль, которая ведёт финальное состояние (по умолчанию `merger-agent`
   после `Merging`).
