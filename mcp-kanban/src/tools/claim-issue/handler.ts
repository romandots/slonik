 ...
 *   4. alias-резолв: ищем колонку, имя которой (в lower-case) попадает в
 *      множество `{ ref, ...aliases }` (тоже в lower-case). Покрывает оба
 *      кейса: (a) `ref` — canonical, а колонка переименована в alias
 *      (`Development` ↔ `Разработка`); (b) `ref` — alias, а колонка
 *      названа canonical-именем.
 *
