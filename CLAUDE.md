...
|---|---|
| [README](./plane/docs/README.md) | Быстрый старт, подключение агентов |
...
├── caddy/                       # Caddyfile для внешнего TLS (Phase 7, пусто)
├── prometheus/, grafana/, loki/, promtail/  # observability (Phase 8, пусто)
└── backup/                      # backup-скрипт (Phase 9, пусто)
```
...
Закрыты Phase 0–5 (см. [ROADMAP.md](./plane/docs/ROADMAP.md) и
...
Ещё впереди: Phase 6 (git integration), 7 (caddy + TLS), 8 (observability),
9 (backup), 10 (hardening + v1.0).

В коде на сегодня — **19 MCP tool'ов**: `who_am_i` + 10 read (`list_*`,
`get_issue`, `search_issues`, `get_issue_history`) + 8 write (`create_issue`,
`update_issue`, `transition_issue`, `claim_issue`, `release_issue`,
`block_issue`, `comment_issue`, `attach_file`). `link_git_ref`/`unlink_git_ref`
из SPEC.md ещё не реализованы.
