...
> конвейера + пример MCP-конфига — лежит в каталоге [`claude/`](../../claude/)
> репозитория (`claude/CLAUDE.md` = этот текст; как развернуть — см.
> [`claude/README.md`](../../claude/README.md)). **При правке этого блока
> синхронизируйте `claude/CLAUDE.md`.**
