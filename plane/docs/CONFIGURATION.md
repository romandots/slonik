...
├── mcp-kanban/
│   ├── Dockerfile
│   ├── bootstrap/manifest.yaml # states/labels/identities
│   └── config/default.yaml     # дефолты MCP, переопределяются ENV
...
| `MCP_DEFAULT_PROJECT` | `code-agents` | slug проекта по умолчанию |
| `MCP_ALLOWED_PROJECTS` | `code-agents` | comma-separated, проекты, к которым MCP даёт доступ |
...
Глобально — `~/.claude/mcp.json`:

```jsonc
{
  "mcpServers": {
    "slonk-kanban": {
      "url": "https://mcp.slonk.example.com/mcp",
      "headers": {
...
В проекте — `.mcp.json` в корне репозитория с той же структурой, но без
секретов; токен — из `~/.claude/.env` (загружается Claude Code).

### 6.2 Codex CLI

`~/.codex/mcp.json` — аналогичная схема. Если Codex не поддерживает HTTP+SSE
для MCP, используется stdio-режим через локальный wrapper:

```bash
mcp-kanban stdio \
  --plane-url http://localhost:8000/api/v1 \
  --plane-api-key $PLANE_API_KEY \
  --identity developer-agent
```

### 6.3 Кастомные MCP-клиенты

Любой клиент, говорящий MCP-over-HTTP, подключается так же. Минимум:
